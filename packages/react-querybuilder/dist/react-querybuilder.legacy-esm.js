...
var numerifyValues = (rg) => __spreadProps(__spreadValues({}, rg), {
  ...
  rules: rg.rules.map((r) => {
    ...
    if (Array.isArray(r.value)) {
      return __spreadProps(__spreadValues({}, r), { value: r.value.map((v) => parseNumber(v, { parseNumbers: true })) });
    }
    const va = toArray(r.value).map((v) => parseNumber(v, { parseNumbers: true }));
    if (va.every((v) => typeof v === "number")) {
      if (va.length > 1) {
        return __spreadProps(__spreadValues({}, r), { value: va });
      } else if (va.length === 1) {
        return __spreadProps(__spreadValues({}, r), { value: va[0] });
      }
    }
    return r;
  })
