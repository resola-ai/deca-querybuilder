...
function formatQuery(ruleGroup, options = {}) {
  ...
  if (format === "json" || format === "json_without_ids") {
    const rg = parseNumbers ? produce2(ruleGroup, numerifyValues) : ruleGroup;
...
var mergeClassnames = (...args) => {
  const joinClassnamesByName = (name) => clsx(args.map((c) => clsx(c?.[name])));
  return {
...
// src/utils/mergeTranslations.ts
var mergeTranslations = (base, ...otherTranslations) => produce3(base, (draft) => {
...
var prepareRule = (rule, { idGenerator = generateID } = {}) => produce4(rule, (draft) => {
  if (!draft.id) {
    draft.id = idGenerator();
  }
});
var prepareRuleGroup = (queryObject, { idGenerator = generateID } = {}) => produce4(queryObject, (draft) => {
  if (!draft.id) {
    draft.id = idGenerator();
  }
...
var remapProperties = (obj, propertyMap, deleteRemappedProperties) => produce6(obj, (draft) => {
...
var useQueryBuilderSetup = (props) => {
  ...
  const [initialQueryProp] = useState2(props.query ?? props.defaultQuery);
...
var useRule = (props) => {
  ...
  const fieldBasedClassName = useMemo4(() => fieldData?.className ?? "", [fieldData?.className]);
...
var useValueEditor = (props) => {
  ...
  const multiValueHandler = useCallback7(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    (val, idx) => {
      const v = produce7(valueAsArray, (va) => {
